...
def read_save(save_name: str) -> Game_Save:
    ...
    with open(config.SAVES_PATH, "r") as file:
        ...
        economy_data = data[save_name].get("economy_data", {})
        inventory_data = data[save_name].get("inventory_data", {})
        team_data = data[save_name].get("team_data", [])
        return Game_Save(save_name, time, coordinates, map, save_name,
                         economy_data, inventory_data, team_data)
...
def write_save(coordinates: tuple[int, int], economy_data: dict = None,
               inventory_data: dict = None, team_data: list = None):
    """
    ...
    with open(config.SAVES_PATH, "r") as file:
        ...
        data[play_name] = {"player_coordinates": [coordinates[0], coordinates[1]],
                           "map": map,
                           "time": 0,
                           "economy_data": economy_data if economy_data is not None else {},
                           "inventory_data": inventory_data if inventory_data is not None else {},
