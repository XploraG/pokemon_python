...
class Menu(Game_Sceene):
    ...
    def update(self) -> None:
        ...
        Updates the menu.
        """
        # Check if there is a pending save loaded from the background thread
        if self.new_save.pending_save is not None:
            save = self.new_save.pending_save
            self.new_save.pending_save = None
            self.load(save)

        # Check if any menu save has a pending load clicked
        for menu_save in self.menu_saves:
            if menu_save.pending_load:
                now = pygame.time.get_ticks()
                if now - menu_save.click_time >= 1000:
                    menu_save.pending_load = False
                    self.load(menu_save.game_save)
    ...
    def render_head_text(self) -> None:
        ...
        text = font.render("Pixel Tamer", True, config.Colors.white)
