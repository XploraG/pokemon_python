...
#       self.change_game_state(Game_State.RUNNING)
    ...
    def render_hud(self) -> None:
        """
        Renders the HUD overlay showing coins, PUSDT, and team info.
        """
        # Coin display background
        hud_rect = pygame.Rect(config.Resolution[0] - 200, 5, 190, 55)
        hud_surface = pygame.Surface((190, 55), pygame.SRCALPHA)
        hud_surface.fill((0, 0, 0, 160))
        self.screen.blit(hud_surface, hud_rect)
        pygame.draw.rect(self.screen, '#FFD700', hud_rect, 2, border_radius=5)

        # Coin icon (small yellow circle)
        pygame.draw.circle(self.screen,
                           '#FFD700',
                           (hud_rect.x + 18, hud_rect.y + 18),
                           10)
        pygame.draw.circle(self.screen,
                           '#FFA500',
                           (hud_rect.x + 18, hud_rect.y + 18),
        ...
        coin_text = font.render(
            f'{self.player.economy.get_formatted_coins()}',
            True, '#FFD700')
        self.screen.blit(coin_text, (hud_rect.x + 35, hud_rect.y + 8))

        # PUSDT text
        pusdt_font = pygame.font.Font(config.FONT_PATH, config.FONT_SIZE - 2)
        pusdt_text = pusdt_font.render(
            f'PUSDT: {self.player.economy.get_formatted_pusdt()}',
            True, '#00FF88')
        self.screen.blit(pusdt_text, (hud_rect.x + 10, hud_rect.y + 32))
            ...
            if event.type == pygame.QUIT:
                ...
                # If CRTL s is presed we will save the actual play
                    ...
                    if mods & pygame.KMOD_CTRL:
                        write_save(
                            (self.player.x_position*config.SCALE,
                             self.player.y_position*config.SCALE),
                            self.player.economy.to_save_data(),
                            self.player.inventory.to_save_data(),
                            self.player.team)
